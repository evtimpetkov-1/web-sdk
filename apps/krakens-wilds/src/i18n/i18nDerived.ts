...
export const i18nDerived = {
	...
	overviewDesc: () => t("Kraken's Wilds is a 5-reel, 3-row video slot with 20 fixed {0}. Wins are evaluated from left to right on each {1}, starting from the leftmost reel. Only symbols on adjacent reels count. The highest win per {1} is {2}."),
	betModeRtp: () => t('Base game: RTP 96.50%. Maximum win: 5,000x total {0}.'),
	betModeMultiplier: () => t('All {0} values are shown as multipliers of the total {1} amount.'),
	wildSymbolDesc: () => t('The Wild (Kraken) substitutes for all symbols except the Bonus symbol. Wild has its own {0} values.'),
	...
	grHighestWin: () => t('Only the highest win per {0} is {1}.'),
	grSimultaneous: () => t('Simultaneous wins on different {0} are added together.'),
	grLineBonus: () => t('{0} wins and Bonus wins are added together.'),
	grBonusAny: () => t('Bonus symbols {0} in any position (not limited to {1}).'),
	// GameRules — UI guide
	uiSpin: () => t('Spin — Start a spin at the current {0} amount.'),
	...
	uiPlusMinus: () => t('+/− — Increase or decrease the {0} amount.'),
	uiMenu: () => t('Menu (☰) — Open the settings menu.'),
	uiPaytable: () => t('{0} — View symbol {1} values and {2}.'),
